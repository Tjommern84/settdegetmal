...
-- Add a geography column so services can store their primary location.
ALTER TABLE services
  ADD COLUMN IF NOT EXISTS base_location geography(Point, 4326);

- Normalize searchable text for faster fuzzy filtering.
ALTER TABLE services
...
-- Normalized coverage rules for fast spatial filters.
...
-- Migrate existing JSON coverage definitions into the normalized table.
WITH extracted AS (
  SELECT
    s.id AS service_id,
    cov.value ->> 'type' AS type,
    cov.value -> 'center' AS center,
    (cov.value ->> 'radius_km')::numeric AS radius_km,
    cov.value -> 'cities' AS cities,
    cov.value ->> 'region' AS region
  FROM services s
...
SELECT
  ...
  CASE
    WHEN type = 'radius'
         AND center IS NOT NULL THEN
      ST_SetSRID(ST_MakePoint(
        ((center ->> 'lon')::double precision),
        ((center ->> 'lat')::double precision)
      ), 4326)
    ELSE NULL
  END,
  radius_km,
...
FROM extracted
LEFT JOIN LATERAL (
  SELECT jsonb_array_elements_text(cities) AS city
  FROM (SELECT cities) AS arr
  WHERE cities IS NOT NULL
) AS city_value ON type = 'cities'
WHERE type IS NOT NULL
ON CONFLICT DO NOTHING;

-- Indexes for service coverage lookups.
...
-- Search function that delegates matching and ranking to the database.
...
BEGIN
  ...
  FROM (
    SELECT
      *,
      ...
      distance_score,
      CASE venue_key
        WHEN 'home' THEN 'Hjemme'
        WHEN 'gym' THEN 'Senter'
        ELSE NULL
      END AS venue_label
    FROM (
      SELECT
        *,
        (goal_candidate = 'any'
          OR goal_candidate = ''
          OR goal_candidate::text = ANY(goals)) AS goal_match,
        (type_candidate = 'any'
          OR type_candidate = ''
          OR type_candidate::text = type) AS type_match,
        (budget_candidate = 'any'
          OR budget_candidate = ''
          OR budget_candidate::text = price_level) AS budget_match,
        (
          venue_key IS NOT NULL
          AND venue_key = ANY(venues)
        ) AS venue_match,
        CASE
          WHEN rating_avg >= 4.7 THEN 3
          WHEN rating_avg >= 4.4 THEN 2
          WHEN rating_avg >= 4.1 THEN 1
          ELSE 0
        END AS rating_score,
        CASE
          WHEN distance_km IS NULL THEN 0
          WHEN distance_km <= 5 THEN 3
          WHEN distance_km <= 15 THEN 2
  ...
  WHERE rs.goal_match
    ...
    OR rs.budget_match
    OR rs.venue_match
    OR rs.distance_score > 0
    OR rs.rating_score > 0
  ORDER BY
    CASE sort_mode
